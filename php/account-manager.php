...
//Updating status to 'inactive' and adding the balance for all haystack_users after 48 hours:
$gen_stmt = $pdo->prepare("SELECT * FROM haystack_users WHERE mining_status = ? LIMIT ?, ?");
...
foreach($gen_data as $gd) {
    if (strtotime($gd->mining_start_time) <= (time() - (48*60*60))) {
        $gen_updt = $pdo->prepare("UPDATE haystack_users SET mining_status = ?, total_amount_mined = ? WHERE user_id = ?");
