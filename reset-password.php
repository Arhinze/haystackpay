...
if (isset($_POST["email"])) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM haystack_users WHERE user_email = ?");
...
if (isset($_POST["code"])) {
    ...
    //if(password_hash((int)$_POST["code"], PASSWORD_BCRYPT, ["cost"=>15]) == $_POST["something"]){
        if($_POST["password1"] == $_POST["password2"]){
            //Update Password:
            $us = $pdo->prepare("UPDATE haystack_users SET `password`=? WHERE user_email = ?");
...
<div class="sign-in-box">
    <div style="margin-bottom:24px">
        ...
        <b style="color:#ff9100"><?=$site_name?></b>
    </div>
